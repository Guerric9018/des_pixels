...
template <buffer_description D>
void applyForces(Mesh& mesh, Window& window, draw_info<D> const line_info)
{
	...
	auto draw_current_state = [&](const std::vector<vec2<float>>& vert, bool stay_visible) {
		...
		window.run([&] {
			draw_settings.resize(0);
			draw_n(line_info, lines, vec4<float>(1.0f, 0.7f, 0.8f, 1.0f), GL_LINES);
			for (const auto &ds : draw_settings) {
				call_draw(ds);
			}
		}, stay_visible);
...
int main(int argc, char **argv)
{
	...
	applyForces(mesh, window, line_info);
